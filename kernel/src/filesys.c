...
struct inode *follow_path(struct inode *root, char *path) {
    ...
    if (root->fs_num == 0 && root->dev == 0 && root->ext_mount == NULL)
        working = root;
    else {
        working = alloc_inode();
        ...
        while (path[done_path] != '\0') {
            ...
            if (working != NULL) {
                if (working->ext_mount != NULL) {
                    {
                        char tmp[2] = { 0 };
                        kprint("Switching from \"");
                        kprint(part_path);
                        kprint("\" on mount ");
                        tmp[0] = '0' + working->fs_num;
                        kprint(tmp);
                    }
                    ...
                    ino_t ino = fs_tab[working->fs_num]->fs_ops.find_ino(part_path);
                    if (ino > 0) {
                        working->ino = ino;
                        fs_tab[working->fs_num]->fs_ops.read_inode(working);
                    } else {
...
size_t get_path_unit(char *part_path, char *path) {
    ...
    *part_path = '\0';
    ...
    {
        char tmp[2] = { 0 };
        tmp[0] = '0' + strlen(part_path_base) - part_len;
        kprint(tmp);
    }

    return strlen(part_path_base) - part_len; // Optimise
}
